...
int calibrate_sensor(int fd, uint8_t *plaintext, uint8_t *key,
    ...
    for(; idf<64; idf++){
        ...
        printf("idc idf_clk idf_A %d %d %d\n", idc, idf_clk, idf_A);

        set_idc(idc_idf, idc, 12);
        set_idf(idc_idf, idf_clk, idf_A);
        ...
        // show the fifth sample
        for (int i = 0; i < 16; i++) {
            printf("%02X", sensor_trace[5*16+i]);
        }
        printf("\n");
        int current = 0;
        current += (count_one((int)sensor_trace[5*16+10])+count_one((int)sensor_trace[5*16+11])+count_one((int)sensor_trace[5*16+12]));
        ...
        if(phase_zero&&!phase_FF&&current>20){
            phase_FF = true;
            printf("best idf found idf_clk:%d idf_A:%d\n ", idf_clk, idf_A);
            best_idc = idc;
            best_idf = idf;
            best_idf_clk = idf_clk;
            best_idf_A = idf_A;
            break;
        }
    }
    ...
    set_idf(idc_idf, best_idf_clk, best_idf_A);
...
void set_idf(unsigned char idc_idf[16], int idf, int idf_A) {
    // we have to compute the number of 1s
    int i = 0;
    char buffer[12];
/**
...
int read_trace_w(int fd, uint8_t *sensor_traces, uint8_t *signal_traces) {
    ...
    size_t temp_index = 0;
    for(size_t i=0;i<NUM_SAMPLES;i++){
        sensor_traces[i*16] = temp[num_B - 18*i-2];
        sensor_traces[i*16+1] = temp[num_B - 18*i-3];
        sensor_traces[i*16+2] = temp[num_B - 18*i-4];
        sensor_traces[i*16+3] = temp[num_B - 18*i-5];
        sensor_traces[i*16+4] = temp[num_B - 18*i-6];
        sensor_traces[i*16+5] = temp[num_B - 18*i-7];
        sensor_traces[i*16+6] = temp[num_B - 18*i-8];
        sensor_traces[i*16+7] = temp[num_B - 18*i-9];
        sensor_traces[i*16+8] = temp[num_B - 18*i-10];
        sensor_traces[i*16+9] = temp[num_B - 18*i-11];
        sensor_traces[i*16+10] = temp[num_B - 18*i-12];
        sensor_traces[i*16+11] = temp[num_B - 18*i-13];
        sensor_traces[i*16+12] = temp[num_B - 18*i-14];
        sensor_traces[i*16+13] = temp[num_B - 18*i-15];
        sensor_traces[i*16+14] = temp[num_B - 18*i-16];
        sensor_traces[i*16+15] = temp[num_B - 18*i-17];
        signal_traces[i*2] = temp[num_B - 18*i];
        signal_traces[i*2+1] = temp[num_B - 18*i-1];
    }
